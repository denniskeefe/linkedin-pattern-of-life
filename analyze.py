...
GRAPHIC = HERE / "graphic.html"

# The poster carries its data in script blocks rather than a placeholder, so it
# stays openable on its own while still being a template.
DATA_BLOCK = '<script id="data" type="application/json">'
SUBJECT_BLOCK = '<script id="subject" type="application/json">'
BLOCK_CLOSE = "</script>"
...
def fill_block(html, opening, payload, where):
    """Replace the contents of one <script> data block."""
    start = html.find(opening)
    if start == -1:
        sys.exit(f"{where}: no {opening}…</script> block to fill")
    body = start + len(opening)
    end = html.find(BLOCK_CLOSE, body)
    if end == -1:
        sys.exit(f"{where}: unterminated {opening} block")
    return html[:body] + json.dumps(payload, separators=(",", ":")) + html[end:]


def render_graphic(events, meta, out):
    """The poster view of the same events, rendered for one subject."""
    if not GRAPHIC.exists():
        return None

    html = GRAPHIC.read_text()
    html = fill_block(html, DATA_BLOCK, [{
        "t": e["iso"][:16], "k": e["kind"], "r": e["reactions"],
        "c": e["comments"], "p": e["reposts"], "m": e["media"], "x": e["text"],
    } for e in events], GRAPHIC.name)
    html = fill_block(html, SUBJECT_BLOCK, {
        "label": meta["label"],
        "subject": meta["subject"],
        "url": meta["url"],
        "thirdParty": not meta["self"],
    ...
    out.write_text(html)
    print(f"wrote {out.relative_to(HERE)}  ({len(events)} events)")
    return out
...
def main():
    ...
    render_graphic(events, meta, d / "graphic.html")
